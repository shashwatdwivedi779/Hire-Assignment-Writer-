    ...
    try {
        ...
        if (user.role === 'client' && assignment.student._id.toString() === user._id.toString()) {
            ...
            for (const prop of proposals) {
                if (prop.writer && prop.writer.avatar) {
                    prop.writer.avatar = await getSignedUrl(prop.writer.avatar);
                }
            }
        } else if (user.role === 'writer') {
        ...
        if (assignment.student && assignment.student.avatar) {
            assignment.student.avatar = await getSignedUrl(assignment.student.avatar);
        }
        if (assignment.assignedWriter && assignment.assignedWriter.avatar) {
            assignment.assignedWriter.avatar = await getSignedUrl(assignment.assignedWriter.avatar);
        }

        // Sign submission files
        for (const sub of assignment.submissions || []) {
            if (sub.writer && sub.writer.avatar) {
                sub.writer.avatar = await getSignedUrl(sub.writer.avatar);
            }
...
exports.GetProfile = async (req, res) => {
    try {
        ...
        if (profile.avatar) {
            profile.avatar = await getSignedUrl(profile.avatar);
        }
...
exports.PostProfileUpdate = async (req, res) => {
    try {
        ...
        if (req.file) {
            const uploaded = await uploadFile(req.file, "profile");
            
            // Delete old avatar from S3 if exists
            const currentUser = await Users.findById(req.userId).lean();
            if (currentUser && currentUser.avatar) {
                await deleteFile(currentUser.avatar);
            }

            updateData.avatar = uploaded.key;
        }
